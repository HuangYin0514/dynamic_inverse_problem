...
class PINN_two_input_port(nn.Module):
    def __init__(self, config, logger, *args, **kwargs):
        super(PINN_two_input_port, self).__init__()
    ...
    def criterion(self, data, *args, **kwargs):
        ...
        y0_tile = torch.tile(y0, (t0.shape[0], 1))
        q_hat, qt_hat, qtt_hat = self.get_q_qt_qtt(t0, y0_tile)
        ...
        y0_tile = torch.tile(y0, (train_t.shape[0], 1))
        q_hat, qt_hat, qtt_hat = self.get_q_qt_qtt(train_t, y0_tile)
        loss_data = 0
        ...
        y0_tile = torch.tile(y0, (physics_t.shape[0], 1))
        q_left_hat, qt_left_hat, qtt_left_hat = self.get_q_qt_qtt(physics_t, y0_tile)
    ...
    def evaluate(self, data, output_dir="", current_iterations=None, *args, **kwargs):
        ...
        #
        ################################
        y0_tile = torch.tile(y0, (data_t.shape[0], 1))
        q_hat, qt_hat, qtt_hat = self.get_q_qt_qtt(data_t, y0_tile)
